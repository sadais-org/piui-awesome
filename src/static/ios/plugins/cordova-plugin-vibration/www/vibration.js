cordova.define('cordova-plugin-vibration.notification', function(require, exports, module) {
  ...
  module.exports = {
    ...
    vibrate: function(param) {
      ...
      // vibrate
      if (typeof param == 'number' && param !== 0) exec(null, null, 'Vibration', 'vibrate', [param])
      // vibrate with array ( i.e. vibrate([3000]) )
      else if (typeof param == 'object' && param.length == 1) {
        // cancel if vibrate([0])
        if (param[0] === 0) exec(null, null, 'Vibration', 'cancelVibration', [])
        // else vibrate
        else exec(null, null, 'Vibration', 'vibrate', [param[0]])
      }
